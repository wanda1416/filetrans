...
public class Test {
	...
	public static void main(String arg[]) throws Exception {
		...
		hostManager.setHostListener(new HostListener() {
			...
			public void onAdd(HostInfo info) {
				...
				remote = new InetSocketAddress(info.getAddress(), info
						.getPort());
			}
		...
		IReceiveListener receiver = new ReceiveAdapter() {
			...
			public void onSchedule(FileHeader header, long sentBytes,
				...
				if (r > 100) {
					transfer.getReceiveServer().abort(header.getFileID());
				}
			}
		};
		...
		ISendListener sender = new ISendListener() {
			...
			public void onSchedule(FileHeader header, long sentBytes,
				...
				if (r > 100) {
					transfer.getSendServer().abort(header.getFileID());
				}
			}
		};
	...
	public static void testConfig(Configuration config) {
		println(config.getReceivePath());
		println(config.getLocalPort());
		println(config.getLocalAddress());
		println(config.getMaxBacklog());
	}
